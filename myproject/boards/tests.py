...
class NewTopicTests(TestCase):
    def setUp(self):
        ...
        # create user instance to use in tests
        User.objects.create_user(username='john', email='john@doe.com', password='1234')
    
    # check if the request to the view is succesful
    def test_new_topic_view_success_status_code(self):
        url = reverse('new_topic', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    # check if the view is raising a 4040 error when the Board does not exist
    ...
    # ensure HTML containts CSRF token
    def test_csrf(self):
        url = reverse('new_topic', kwargs={'pk': 1})
        response = self.client.get(url)
    ...
    # send an empty dictionary to check how the application is behaving
